...
import { ServerUpdateHandlerFake } from './ServerUpdateHandlerFake';
...
export class AppComponent {
  voteLabels: string[] = ["?", ".5", "1", "2", "3", "5", "8", "13", "20", "40", "100"];
  ...
  onJoinLeave() {
    if (!this.isJoined) {
      ...
      const clientRequest: ClientRequest = new ClientRequest(ClientRequest.JOIN, this.voterName);
      this.serverCommunications.sendRequest(clientRequest, (joinedSuccess: boolean) => { this.doAfterJoin(joinedSuccess) });

      return;
    }

    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.LEAVE, this.voterName);
    this.serverCommunications.sendRequest(clientRequest, (leftSuccess: boolean) => { this.doAfterLeave(leftSuccess) });
  }
  ...
  private doAfterJoin(joinedSuccess: boolean): void {
    ...
    this.serverCommunications.createWebSocket(this.voterName, (serverUpdate: ServerUpdate) => this.receiveServerUpdate(serverUpdate));
  }
  ...
  onStartVote() {
    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.START_VOTE, this.voterName);
    this.serverCommunications.sendRequest(clientRequest);
  }

  onCloseVote() {
    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.END_VOTE, this.voterName);
    this.serverCommunications.sendRequest(clientRequest);
  }

  onCancelVote() {
    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.CANCEL_VOTE, this.voterName);
    this.serverCommunications.sendRequest(clientRequest);
  }

  onRefresh() {
    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.REFRESH, this.voterName);
    this.serverCommunications.sendRequest(clientRequest);
  }

  onVote(vote: number) {
    const clientRequest: ClientRequest = new ClientRequest(ClientRequest.VOTE, this.voterName, vote);
    this.serverCommunications.sendRequest(clientRequest);
  }
