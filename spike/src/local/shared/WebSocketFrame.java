...
public class WebSocketFrame {
	...
	public WebSocketFrame(final boolean finishedParm, final WebSocketOpCode opCodeParm, final boolean maskedParm, final byte[] payloadParm) {
		...
		if (payloadLength > 0xA00) {
			errorMessage = "Payload length is greater than 10k.";
			mask = new byte[] { 0, 0, 0, 0 };
			readOnlyBuffer = ByteBuffer.allocate(0);
	 ...
	 *            required, must be flipped, no side-effects
	 */
	public WebSocketFrame(final ByteBuffer requestBufferParm) {
		asserts(requestBufferParm != null, "Request ByteBuffer is null.");
		asserts(requestBufferParm.position() == 0, "Request ByteBuffer not flipped (positon not 0): " + requestBufferParm.position());
